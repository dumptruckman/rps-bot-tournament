...
- [x] `rps-tournament present --directory TOURNAMENT_DIRECTORY` starts a
...
- [x] The process starts while `scoreboard.json` is absent, but rejects an
  unreadable Tournament directory, an invalid bind address, and an unavailable
  port with a non-zero exit.
- [x] A version-aware, allowlisted browser contract exposes only Tournament
...
- [x] The browser polls once per second with conditional requests and replaces
  its whole view for each accepted atomic projection rather than merging
  generations.
- [x] Missing, corrupt, unreadable, or unsupported projections retain the last
  valid view and produce a visible local freshness warning; recovery clears the
  warning without altering competitive status.
- [x] No presentation module imports scoring, standings calculation,
...
- [x] Bundled HTML, CSS, and vanilla JavaScript load without a CDN, asset build,
  framework, or event-day runtime dependency beyond Python 3.9 and its standard
  library.
- [x] Contract and HTTP integration tests cover paused and running views, ETag
...
Implemented a dedicated loopback presentation command, a versioned and
allowlisted live standings contract, a read-only HTTP server with last-valid
projection retention and conditional requests, and bundled responsive browser
assets. Added contract, CLI, package-boundary, and HTTP integration coverage for
